...
export async function buildLogoutUrl(
  ...
  const urlObj = oidc.buildEndSessionUrl(config, {
    ...
    post_logout_redirect_uri: ZITADEL_POST_LOGOUT_URL,
    state,
  });
 ...
 * page.data.session object and load functions.
 */
...
export const { handle, signIn, signOut } = SvelteKitAuth({
  ...
  session: {
    strategy: 'jwt',
    maxAge: Number(SESSION_DURATION) || 3600,
  },
  ...
  callbacks: {
     ...
     *
     * ## When This Runs
     * - Every time a page loads with session data
     ...
     * that your application code can access via page.data.session in load functions.
     *
     * ## Security Note
     *
